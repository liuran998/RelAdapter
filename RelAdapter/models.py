...
class MetaR(nn.Module):
    ...
    def forward(self, task, adaptor,iseval=False, curr_rel=''):
   ...
   # def validation_adaptor(self, task, adaptor,iseval=False, curr_rel=''):
        # transfer task string into embedding
        # support, support_negative, query, negative = [self.embedding(t) for t in task]
        # support, support_negative, query, negative = [get_params.adaptor(self.embedding(t)) for t in task]
   #     support, support_negative, valid , valid_negative = [self.embedding(t) for t in task]
 ...
 #           else:
  #              rel_q = rel
   ...
   #         self.rel_q_sharing[curr_rel] = rel_q
        ...
        # rel_q = adaptor(rel_q)
    #    rel_q = rel_q.expand(-1,num_v + num_v_neg, -1, -1)

     #   que_neg_e1, que_neg_e2 = self.split_concat(valid, valid_negative)
      #  p_score, n_score = self.embedding_learner(que_neg_e1, que_neg_e2, rel_q, num_v)
